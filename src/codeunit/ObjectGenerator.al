codeunit 91003 DAMObjectGenerator
{
    ...
    begin
        ...
        C.AppendLine('  {');
        C.AppendLine('    Date=09.10.21;');
        C.AppendLine('    Time=22:43:28;');
        ...
        C.AppendLine('                                                   CaptionML=[DEU=Order für Tabellendaten;');
        ...
        C.AppendLine('                                                   CaptionML=[DEU=Tabellen-ID Filter für Export;');
        ...
        C.AppendLine('       ERROR(STRSUBSTNO(''Ungültiger Ordnerpfad: %1'',FolderPathToCheck));');
        ...
        C.AppendLine('        Progress.OPEN(STRSUBSTNO(''Export %1'',CONVERTSTR(ExportToPath,''\'',''/'')));');
        C.AppendLine('        EXPORT_ExportFullTable(ExportToPath,AllObjWithCaption."Object ID",FALSE,ExportDuration,TRUE);');
        C.AppendLine('        ReadyMsg += STRSUBSTNO(''\%1: %2'',AllObjWithCaption."Object Caption",ExportDuration);');
        C.AppendLine('        Progress.CLOSE;');
        C.AppendLine('      UNTIL AllObjWithCaption.NEXT = 0;');
        C.AppendLine('      MESSAGE(ReadyMsg)');
        ...
        C.AppendLine('    END;');
        C.AppendLine('');
        C.AppendLine('    PROCEDURE EXPORT_ExportFullTable@1109900001(ExportToFilePath@1000000001 : Text[1024];ExportTableID@1000000004 : Integer;ExportWithTitleLine@1000000007 : Boolean;VAR ExportDuration@1000000003 : Duration;AddTableCaptionColAndAllFields@1000000010 : Boolean);');
        C.AppendLine('    VAR');
        ...
        C.AppendLine('          IF (_ExportFieldBuffer.TableNo = DATABASE::Field) AND (_ExportFieldBuffer."No."=50000) THEN BEGIN');
        ...
        C.AppendLine('          END ELSE');
        C.AppendLine('            _ValueAsText := EXPORT_FormatFieldValue(_RecRef,_ExportFieldBuffer."No.");');
        C.AppendLine('');
        C.AppendLine('          // Remove FieldSeparator from Value');
        C.AppendLine('          _ValueAsText := ReplaceStr(_ValueAsText,_FieldSeparator,'''');');
        C.AppendLine('          IF _ExportFieldBuffer.Type = _ExportFieldBuffer.Type::Boolean THEN');
        C.AppendLine('            IF NOT (_ValueAsText IN [''0'',''1'']) THEN ERROR(''fail'');// else _ValueAsText := ''true/false'';');
        ...
        C.AppendLine('            //_LineText += _ValueAsText + '','';');
